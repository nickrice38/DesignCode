...
//  DesignCode
//
//  Created by Nick Rice on 17/01/2022.
//
        ...
        Text("Button")
            ...
            .gesture(
                LongPressGesture(minimumDuration: 0.5, maximumDistance: 10).onChanged { value in
                    self.tap = true
                    impact(style: .heavy)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { // this gives us a 0.1 sec delay
                        self.tap = false
                    }
                }
                    .onEnded { value in
...
struct PayButton: View {
    ...
    var body: some View {
        ZStack {
            ...
            Image("fingerprint-2")
                .clipShape(Rectangle().offset(y: tap ? 0 : 50)) // 0 means show the entire image
                .animation(.easeInOut)
                .opacity(press ? 0 : 1)
                .scaleEffect(press ? 0 : 1)
        ...
        .background(
            ...
            .overlay(
                Circle()
                    .trim(from: tap ? 0.001 : 1, to: 1) // the lower the number the more complete
